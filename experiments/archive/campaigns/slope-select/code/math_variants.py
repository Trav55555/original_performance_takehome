...
def tile_first(ir, scalars, ids, bits, order):
    ...
    from linear_coefficients import coefficients as algebraic, structural_slope
    shape = CFG.get('shape', 'tensor')
    sharing = CFG.get('sharing', 'structural')
    assert shape in ('tensor', 'slope') and sharing in ('structural', 'algebraic')
    if sharing == 'algebraic':
        a, h, v, fourth = algebraic(ir, scalars, ids, shape)
    elif shape == 'slope':
        a, h, v, fourth = structural_slope(ir, scalars, ids)
    else:
        a, h, v, fourth = coefficients(ir, scalars, ids)
    base = ir.emit('muladd', args=(bits[0], h, a))
    slope = (ir.emit('select', args=(bits[0], fourth, v)) if shape == 'slope'
             else ir.emit('muladd', args=(bits[0], fourth, v)))
    if not hasattr(ir, 'tile_traces'):
        ir.tile_traces = []
    ir.tile_traces.append({'ids': ids, 'bits': bits, 'base': base, 'slope': slope,
                           'coefficients': [a, h, v, fourth], 'shape': shape})
    return [base, slope]
