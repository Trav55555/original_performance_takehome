...
class _IR:
    ...
    def emit(self, kind, code="", args=(), width=VLEN, imm=None, preferred="valu"):
        ...
        self.ops[-1].coord = getattr(self, 'coord', None)
        ...
        for b in range(blocks):
            for r in range(begin, min(begin + 12, _ROUNDS)):
                ...
                if r == 0:
                    ...
                    if variants.full_enabled(b, r, depth):
                        bs = [history[b][-1 - k] for k in bit_order]
                        entries = [variants.full_lookup(ir, scalars, ids, bs)]
                        bit_order = []
                        matched.update(s for s in requested if s[:2] == (b, r))
                    for layer, bit_index in enumerate(bit_order):
                        ...
                        for j in range(0, len(entries), 2):
                            site = (b, r, layer, j // 2)
                            tile = j // 4 if layer == 0 else j // 2
                            order = variants.tile_order(b, r, tile)
                            if depth in (3, 4) and order is not None and layer in (0, 1):
                                if layer == 0:
                                    if j % 4 == 0:
                                        bs = [history[b][-1 - k] for k in list(bit_order)[:2]]
                                        next_entries.extend(variants.tile_first(ir, scalars, ids[j:j+4], bs, order))
                                        matched.update(s for s in requested if s[:3] == (b, r, 0) and s[3] in (j//2, j//2+1))
                                else:
                                    last_bit = history[b][-1 - list(bit_order)[0 if order else 1]]
                                    next_entries.append(ir.emit('muladd', args=(last_bit, entries[j+1], entries[j])))
                                continue
                ...
                for h, (op1, c1, op2, op3, c3) in enumerate(HASH_STAGES):
                    ...
                    if advanced and h == 2:
                        ...
                        if variants.radix_enabled(b, r):
                            branch_override = variants.branch(ir, affine_input, shift)
                        if decode_here:
                ...
                if r + 1 < _ROUNDS:
                    if depth == _HEIGHT:
                        ...
                        bit = branch_override or ir.binary("&", v, ir.vc(1), preferred="alu")
...
def _schedule(ir: _IR, lookahead=False, startup=False):
    ...
    priority = variants.geometric_priority(ir, position, priority)
