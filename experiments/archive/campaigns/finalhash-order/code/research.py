...
CAPS = {'candidate':8, 'control':4, 'validation':2}
...
def reserve(arm, name):
    guard()
    assert json.loads((HERE/'manifest.json').read_text())['status'] == 'ready', 'budget closed'
...
def build_ir(cfg, module=compiler):
    ...
    return module._build_ir(tuple(map(tuple, PLAN['sites'])), advanced=True,
        pairs=tuple(PLAN['pairs']), final_blocks=tuple(cfg.get('final_blocks', PLAN['final_blocks'])),
...
def row_for(arm, name, cfg, built):
    ...
    if program is not None:
        ...
        row['shared_coefficients'] = getattr(ir, 'shared_coefficients', 0)
