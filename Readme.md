...
- **1113 cycles**: This repo (XOR-encoded values, mirrored tree indices, 32 active vector blocks, dependency scheduling with load lookahead, and a co-issued terminal pause; previously 1303 cycles)
...
The retained kernel takes **1,113 cycles** for height 10, 2,047 tree nodes, batch size 256, and 16 rounds. This saves 190 cycles from the previous 1,303-cycle baseline, a 14.6% reduction. Scratch usage is **1,416 of 1,536 words**, on one core with eight SIMD lanes.

Resource-aware bounds put the optimum for the current dependency graph between 1,104 and 1,113 cycles. This is not a bound on every legal implementation. See [the scheduling experiment](experiments/scheduling_bounds_results.md) for the derivation and [the optimization history](experiments/retry_results.md) for the representation changes.
