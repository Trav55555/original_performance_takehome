...
- **1084 cycles**: This repo (encoded values, folded root initialization, mirrored indices, predicate reuse, selective depth-4 caching, and dependency scheduling with load lookahead; previously 1303 cycles)
...
The retained kernel takes **1,084 cycles** for height 10, 2,047 tree nodes, batch size 256, and 16 rounds. This saves 219 cycles from the previous 1,303-cycle baseline, a 16.8% reduction. Scratch usage is **1,536 of 1,536 words**, on one core with eight SIMD lanes.

Resource-aware bounds put the optimum for the current dependency graph between 1,074 and 1,084 cycles. This is not a bound on every legal implementation. See [the domain sweep](experiments/domain_sweep_results.md) for the latest changes and bound, [the predicate and selective-cache experiments](experiments/cross_domain_results.md) for the preceding graph change, and [the optimization history](experiments/retry_results.md) for the representation changes.
